...
from src.application.schemas import JobRequest, AddJobSuccessfully, StopJobSuccessfully
...
async def create_job(job: JobRequest, background_tasks: BackgroundTasks, task_service: Annotated[TaskService, Depends(Dependencies.task_service)]) -> AddJobSuccessfully:

    if await task_service.is_exist(job.job_id):
